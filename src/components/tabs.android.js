'use strict'
import React, { Component } from 'react'
import TabNavigator from 'react-native-tab-navigator'
import {
    Text,
    View,
    TouchableHighlight,
    // Alert,
    StyleSheet,
    Image
} from 'react-native'
import Instance from './instanceView'
import Logout from './logoutView'
import Alert from './alertView'

const iconDashboard = 'data:image/pngbase64,'

const iconHeroes = 'data:image/pngbase64,'

class Tabs extends Component {
  constructor(props) {
    super(props)
    this.state = {selectedTab: 'instance'}
  }

  render() {
    return (
      <TabNavigator>

        <TabNavigator.Item
          selected={this.state.selectedTab === 'instance'}
          renderIcon={() =>
            <Image
              source={{uri: iconDashboard}}
              style={{width: 20, height: 20}} />
          }
          title="Instance"
          onPress={() => this.setState({ selectedTab: 'instance' })}>
          <Instance {...this.props.route} navigator={this.props.navigator} route={this.props.route}></Instance>
        </TabNavigator.Item>

        <TabNavigator.Item
          tabBarStyle={{width: 0, height: 0}}
          selected={this.state.selectedTab === 'alert'}
          renderIcon={() =>
            <Image
              source={{uri: iconDashboard}}
              style={{width: 20, height: 20}} />
          }
          title="Alert"
          onPress={() => this.setState({ selectedTab: 'alert' })}>
          <Alert {...this.props.route} navigator={this.props.navigator} route={this.props.route}></Alert>
        </TabNavigator.Item>

        <TabNavigator.Item
          selected={this.state.selectedTab === 'logout'}
          renderIcon={() =>
            <Image
              source={{uri: iconHeroes}}
              style={{width: 25, height: 25}} />
          }
          title="Logout"
          onPress={() => this.setState({ selectedTab: 'logout' })}>
          <Logout></Logout>
        </TabNavigator.Item>

      </TabNavigator>
    )
  }
}
module.exports = Tabs
